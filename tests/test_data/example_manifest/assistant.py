...
class ResearchAssistant:
    ...
    def __init__(
        ...
        self.reports_config_path = self.environment_sys.get("used_path", reports_config_path)
        ...
        if query_sys is None and self.environment_sys.get("onprem_enabled"):
            try:
                ...
                self.environment_sys.warn("Failed to initialize on-prem query system. Will not be able to retrieve data from on-prem sources.")
    ...
    def get_reports_config_base(self) -> pd.DataFrame:
        ...
        reports_config = reports_config[list(renamed_azdo_columns.values()) + columns_parsed_from_description]
    ...
    def add_report_ids(
        ...
        is_update = (
            reports_config["predecessor_azdo_id"].notna()
            & reports_config["predecessor_azdo_id"].isin(reports_config.index)
        )
        original_report_wis = reports_config.loc[~is_update].copy()

        # Base the default report IDs on the azdo_id by default
        original_report_wis["report_id"] = original_report_wis["azdo_id"].apply(lambda x: f"ra{x:07d}")
        ...
        if has_invalid_irb.any():
            ...
            reports_config.loc[has_invalid_irb, "error_log"] += "Report metadata error: invalid IRB "
        ...
        irb_reports["irb_rank"] = irb_reports.groupby("irb_number")["azdo_id"].rank().astype(int) - 1
        irb_reports["report_id"] = irb_reports.apply(lambda row: f"{row['irb_number']}.{row['irb_rank']:03d}", axis="columns")
        original_report_wis.loc[irb_reports.index, ["irb_rank", "report_id"]] = irb_reports[["irb_rank", "report_id"]]

        # Update the original data
        reports_config.loc[original_report_wis.index, "report_id"] = original_report_wis["report_id"]
        ...
        g = nx.from_pandas_edgelist(updated_report_wis, source="predecessor_azdo_id", target="azdo_id")
        for component in nx.connected_components(g):
            component_azdo_ids = np.array(sorted(list(component)))
            # Updates inherit the report_id of the original
            reports_config.loc[component_azdo_ids, "report_id"] = reports_config.at[component_azdo_ids[0], "report_id"]
    ...
    def add_report_locations(self, reports_config: pd.DataFrame) -> pd.DataFrame:
        ...
        reports_config.loc[no_location & has_irb, workspace_cols] = ["Research Core", "Research IRB", ""]
        reports_config.loc[no_location & ~has_irb, workspace_cols] = ["Research Operations", "Research Analytics Internal", "Main"]
        ...
        for col in workspace_cols:
            ...
            reports_config.loc[is_not_valid, "error_log"] += f"Report metadata error: Invalid {col}. "

        return reports_config
    
    def get_valid_report_locations(self, relative_path: str = "data/metadata/research_workspaces.csv"):
    ...
    def add_report_dir_patterns(
        ...
        def get_report_dir_pattern(row: pd.Series) -> str:
            ...
            if not pd.isna(row["workspace_folder"]):
                report_dir_pattern += "/" + self.utils_sys.sanitize_dirname(row["workspace_folder"])
        ...
        reports_config["report_dir_pattern"] = reports_config.apply(get_report_dir_pattern, axis="columns")
    ...
    def finalize_reports_config(self, reports_config: pd.DataFrame) -> pd.DataFrame:
        ...
        reports_config.loc[is_active_is_null, "is_active"] = is_active_defaults.loc[is_active_is_null]

        # Turn off invalid reports
        reports_config["is_active"] = reports_config["is_active"] & reports_config["is_valid"]
        ...
        if "depends_on_onprem" not in reports_config.columns:
            ...
            reports_config["depends_on_onprem"] = reports_config["depends_on_onprem"].fillna(True).astype(bool)
        # If on-prem is not enabled, turn off the reports that depend on it
        if not self.environment_sys.get("onprem_enabled"):
            reports_config["is_active" ] = reports_config["is_active"] & ~reports_config["depends_on_onprem"]

        # Only keep the most-recent active report as active
        actually_active_azdo_ids = reports_config.query("is_active").groupby("report_id")["azdo_id"].idxmax()
        ...
        has_irb = reports_config["irb_number"].notna()
        reports_irbs = self.spark.createDataFrame(reports_config.loc[has_irb, ["irb_number"]])
        studies = self.spark.table("research_dm.eirb.studies").filter("(approved_date_calc is not NULL) and (closed_date is NULL)")
        irb_is_active = reports_irbs.join(
            studies.select("irb_study_id"),
            on=(reports_irbs.irb_number == studies.irb_study_id),
            how="left"
        ).toPandas()["irb_study_id"].notna().values
        reports_config.loc[has_irb, "irb_is_active"] = irb_is_active

        # Make active reports visible, and turn off visibility for reports with an IRB where the IRB is inactive
        reports_config["is_visible"] = reports_config["is_active"]
        reports_config.loc[has_irb, "is_visible"] = reports_config.loc[has_irb, "is_visible"] & reports_config.loc[has_irb, "irb_is_active"]
    ...
    def write_reports_config(
        ...
        """
        Note: A function to build a specific table would not normally be incorporated into ra_lib.
        ...
        self.spark.sql(
            ...
            ) COMMENT "This reports_config table contains one row per Azure DevOps work item associated with a report. For a given report_id there may be multiple azdo_ids, i.e. multiple work items, but at most one should be active at a given time, as recorded by the `is_active` field."
            """
        )
    ...
    def identify_reports_to_refresh(self):
        """
    ...
    def draft_report_structure(
        ...
        if report_dir_label is None:
            report_dir_label = self.utils_sys.sanitize_dirname(report_config["azdo_title"], space_sub="-")
        ...
        if is_study:
            report_structure["report_dir"] = report_dir_str.format(study_dir_label, report_dir_label)
        else:
            report_structure["report_dir"] = report_dir_str.format(report_dir_label)

        # Modify the report dir to account for the location of ra_reports, where all the reports live
        report_structure["report_dir"] = os.path.abspath(f"{ra_reports_path}/{report_structure['report_dir']}")
    ...
    def build_report_structure(
        ...
        relative_ra_lib_setup_path = os.path.relpath(ra_lib_setup_path, start=report_dir)
        ...
        if template_files is not None:
            ...
            report_files = [f"{template_dir}/{report_file}" for report_file in report_files]
        ...
        for report_file_path in report_files:
            ...
            try:
                ...
                modified_notebook_str = notebook_str.replace(str_to_replace, relative_ra_lib_setup_path)
                encoded_notebook = base64.b64encode(modified_notebook_str.encode()).decode()
                ...
                print(f"Tried to copy in the template from {report_file_path} but failed. Exception:\n{e}")
